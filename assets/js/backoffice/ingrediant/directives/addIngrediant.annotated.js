...
.directive('addIngrediant', function(){
    ...
    return {
      	...
      	replace: true,
      	templateUrl: 'js/backoffice/ingrediant/partials/addIngrediant.html',
      	controller:["$state", "$timeout", "$scope", "userService", "$rootScope", "ingrediantService", "fabricantService", "$mdToast", "imageService", "documentService", "tagService", "categoryService", "$q", "Upload", "$sailsSocket", function($state,$timeout,$scope,userService,$rootScope,ingrediantService,fabricantService,$mdToast,imageService,documentService, tagService, categoryService, $q, Upload,$sailsSocket){

      		$scope.touched = false;
      		console.log($state);
      		console.log($state.current.name);
      		if($state.current.name == 'ingrediants.edit'){
      			$scope.touched = true;
      		}
      		$rootScope.$on('$stateChangeStart',function (e,toState,toParams,fromState,fromParams){
      			...
      			if($scope.touched == false){
	      			ingrediantService.remove($scope.formData.id).then(function(data){
						console.log('----------------------------------------------------------');
						$rootScope.$broadcast('ingrediantSelfRemove',data.id);
			  ...
			  // For Safari
			  	...
			  	if($scope.touched == false){
	      			ingrediantService.removeSyncro($scope.formData.id).then(function(data){
						console.log('----------------------------------------------------------');
						$rootScope.$broadcast('ingrediantSelfRemove',data.id);
    		...
    		$scope.searchText = null;
      		...
      		console.log($scope.newIngrediant);
			$scope.formData=$scope.newIngrediant;
			if($state.current.name != 'ingrediants.edit'){
      			// $scope.touched = true;
				$rootScope.$broadcast('ingrediantSelfAdd',$scope.newIngrediant);
      		}

			$sailsSocket.subscribe('ingrediant',function(data){
			        console.log('ON ARTICLE2');
			        console.log(data);
			        if(data.id == $scope.newIngrediant.id)
			        {
			        	console.log('cool');
			...
			$sailsSocket.subscribe('comment',function(data){
			        ...
			        if(data.verb =='addedTo' && data.attribute=="responses"){
						...
						if( index !== -1) {
							ingrediantService.fetchOneComment(data.addedId).then(function(data){
								console.log(data);
			...
			$scope.update=function(attribute){
				...
				ingrediantService.update($scope.formData.id,attrToUpdate).then(function(data){
					console.log('----------------------------------------------------------');
					console.log(data);
			...
			$scope.validateIngrediant=function(bonus){
				$rootScope.startSpin();
				
				ingrediantService.validatePai($scope.formData.id,bonus).then(function(data){
					console.log(data);
			...
			$scope.unvalidateIngrediant=function(raison){
				...
				ingrediantService.unvalidatePai($scope.formData.id,raison,$scope.formData.counterOffer).then(function(data){
					console.log(data);
					if(data.data[0].solded){
						$scope.formData.solded = true; 
					}
			...
			$scope.updateComment=function(id,attribute,value){
				...
				ingrediantService.updateComment(id,attrToUpdate).then(function(data){
					console.log('----------------------------------------------------------');
					console.log(data);
					// console.log($scope.$parent);
			...
			$scope.AddReponse=function(id,tmpRep){
				...
				ingrediantService.addReponse(id,attrToUpdate).then(function(data){
					console.log('----------------------------------------------------------');
					console.log(data);
			...
			$scope.deleteComment=function(id){
				...
				$rootScope.startSpin();
				var attrToUpdate = {};
				ingrediantService.deleteComment(id).then(function(data){
					console.log('----------------------------------------------------------');
					console.log(data);
					...
					// $rootScope.$broadcast('ingrediantSelfChangeDoc',data);
					//  else {
			...
			$scope.removeThis=function(id){
				...
				ingrediantService.remove($scope.formData.id).then(function(data){
					...
					console.log(data);
					console.log(id);
					$rootScope.$broadcast('ingrediantSelfRemove',id);
		 ...
		 //    $scope.autocompleteFabricant =function(query) {
		 //    	console.log(query);
		 //    	fabricantService.searchFabricants(query).then(function(result){
		    		
			//     	console.log(result);
			//     	console.log(typeof(result));
			//     	if(typeof(result)=='Array'){
			//     	 	$scope.fabList = result;
			//     	}else{
			//     	 	$scope.fabList = [];
			//     	}
			//     	return result
		 //    	})
		 //    }
		 //   	function transformFabricant(newTag){
			// 	if (newTag.id){
			// 		return newTag
			// 	}else{
			// 		return {
			...
			fabricantService.getFabList().then(function(data){
   			...
   			$scope.onSelectFab=function(item){
   				...
   				console.log($scope.formData.fabricant);
   				var t = $scope.formData.fabricant ;
   				$scope.formData.fabricant = $scope.formData.fabricant.id;
			...
			$scope.addTag=function(newTag){
				...
				ingrediantService.addTag($scope.formData.id,newTag).then(function(data){
					$rootScope.$broadcast('ingrediantSelfChangeTag',data.parent);
			...
			$scope.removeTag=function(t){
				$rootScope.startSpin();
				ingrediantService.removeTag($scope.formData.id,t).then(function(data){
					console.log(data);
					$rootScope.$broadcast('ingrediantSelfChangeTag',data);
					$rootScope.stopSpin();
				},function(d){
					console.log('EROOR');
				})
			...
			$scope.addCategory=function(newCategory){
				...
				ingrediantService.addCategory($scope.formData.id,newCategory).then(function(data){
					$rootScope.$broadcast('ingrediantSelfChangeCat',data.parent);
			...
			$scope.removeCategory=function(category){
				$rootScope.startSpin();
				ingrediantService.removeCategory($scope.formData.id,category).then(function(data){
					$rootScope.$broadcast('ingrediantSelfChangeCat',data);
					$rootScope.stopSpin();
				},function(d){
					console.log('EROOR');
				})
			...
			$scope.addAuthor=function(newAuthor){
				...
				ingrediantService.addAuthor($scope.formData.id,newAuthor).then(function(data){
					$rootScope.$broadcast('ingrediantSelfChangeAuthorAdd',data);
					// data = data.data;
			...
			$scope.removeAuthor=function(author){
				$rootScope.startSpin();
				ingrediantService.removeAuthor($scope.formData.id,author).then(function(data){
					$rootScope.$broadcast('ingrediantSelfChangeAuthorRemove',data);
					$rootScope.stopSpin();
				},function(d){
					console.log('EROOR');
				})
		        ...
		        if (files && files.length) {
		            ...
		            for (var i = 0; i < $scope.uploadsDocument.length; i++) {
		                if( $scope.uploadsDocument[i].status=='start'){
		                    ...
		                    (function(i){
			                    Upload.upload({
			                        url: '/api/ingrediant/'+$scope.formData.id+'/documents',
			                        data: {files : $scope.uploadsDocument[i].file,filename:'tt',name:'t'}
			                    }).then(function (data) {
			                        // $scope.formData.documents = data.data.parent.documents
			                        console.log(data);
			                        // $rootScope.$broadcast('ingrediantSelfChange',data.data.parent);
			                        $rootScope.$broadcast('ingrediantSelfChangeDoc',data.data.parent);
			                        // $rootScope.$broadcast('ingrediantSelfChange',data.parent);
			...
			$scope.removeDocument=function(doc){
				$rootScope.startSpin();
				ingrediantService.removeDocument($scope.formData.id,doc).then(function(data){
			...
			$scope.removeImg=function(img){
				$rootScope.startSpin();
				ingrediantService.removeImage($scope.formData.id,img).then(function(data){
					...
					// 	$scope.formData.tags.push(tag_with_id);
					// }
					$rootScope.$broadcast('ingrediantSelfChangeImg',data);
					$rootScope.stopSpin();
				},function(d){
					console.log('EROOR');
				})
        	...
        	$scope.uploadImage = function () {
                ...
                (function(indexImage){
                    ...
                    Upload.upload({
                    	...
                    	$rootScope.$broadcast('ingrediantSelfChangeImg',data.data.parent);
			...
			// 	$rootScope.startSpin();
			// 	ingrediantService.removeImage($scope.formData.id,doc).then(function(data){
			// 		var index = _.findIndex($scope.formData.documents, function(o) { return o.id == doc; });
			// 		if( index !== -1) {
			// 			$scope.formData.documents.splice(index, 1);
			// 		}
