angular.module('momi-sliders')
  .directive('sliders', ["articleService", "paramsService", function (articleService,paramsService){
    ...
    return {
		...
		controller:["$timeout", "$scope", "$mdDialog", "$rootScope", "userService", "$filter", "sliderService", "imageService", "documentService", "$sailsSocket", "$stateParams", "$state", "usSpinnerService", "Upload", function($timeout,$scope,$mdDialog,$rootScope,userService,$filter,sliderService,imageService,documentService,$sailsSocket,$stateParams,$state,usSpinnerService,Upload){
	     ...
	     //    			var index = _.findIndex($scope.formData.categories, function(o) { return o.id == data.id; });
			...
			$scope.displayAddPopUp=function(){
			    ...
			    $mdDialog.show({
			      	controller: ["$scope", "$rootScope", "sliderService", function($scope,$rootScope,sliderService){
