angular.module('core')
.factory('fabricantService', ["$http", "$q", "$sailsSocket", "$state", "$auth", function ($http,$q,$sailsSocket,$state,$auth) {
    var service = {};
    service.items=[];  
    ...
    service.fetch= function(sort,page,nbPerPage) {
        ...
        console.log('/fabricant/'+sort+'/'+nbPerPage+'/'+nbPerPage*(page-1));
        $sailsSocket.get('/api/fabricant/'+sort+'/'+nbPerPage+'/'+nbPerPage*(page-1)).success(function (data,status) {
            console.log(data);
            service.items =data;
