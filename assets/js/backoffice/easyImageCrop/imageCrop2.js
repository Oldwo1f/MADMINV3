angular.module('core')
.directive('imageCrop2', [function($compile){
	return {
		restrict: 'A',
	    templateUrl: 'js/backoffice/easyImageCrop/image-crop2.html',
		scope:{
			aspectRatio:'=?',
			displayWidth: '=?',
			displayHeight: '=?',
		...
		controller: ['$scope', '$element', '$window' ,function($scope, $element, $window) {
			...
			var imageCropSelectorDiv = $window.document.getElementById("imageCropSelector2");
			var imageCropSelect;
			...
			$scope.$watch("imgSrc", function(newValue, oldValue){
				if(newValue !== oldValue){
					...
					$window.document.getElementById("cropToolLoading2").style.display= "block";
					var imageCropSelector = $window.document.getElementById("imageCropSelector2");
					...
					$element[0].children[0].onload = function(){
						$window.document.getElementById("cropToolLoading2").style.display = "none";

						imageTag = $window.document.getElementById("imageCropSource2");
...
console.log($scope.currentImageWidth);
				...
				originalImage.src = imageTag.src;
				var imageCropSelector = $window.document.getElementById("imageCropSelecto");
