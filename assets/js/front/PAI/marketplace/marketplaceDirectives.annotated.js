angular.module('pai-marketplace')
  .directive('marketplace', ["articleService", function (articleService){
    ...
    return {
		...
		replace: true,
		templateUrl: 'js/front/PAI/marketplace/marketplace.html',
		controller :["$scope", "projectService", function($scope,projectService){
			...
			$scope.myPagingFunction = function(){
				...
				if($scope.fetching == false){
					...
					projectService.fetch($scope.page).then(function(data){
						...
						// console.log(data);
						if(data.length)
							$scope.itemList = _.concat($scope.itemList,data)
			...
			$scope.calcInitialPrice = function(initialPrice, date){
					date = moment(date);
					var date6 = date.add(6,'months');
					var date12 = date.add(12,'months');
					var date18 = date.add(18,'months');
					var date24 = date.add(24,'months');

					var now = moment();

					var iprice = initialPrice;
					var partClub = 0.3;
					var tmpNumber=0;
					if( now.isBetween(date,date6) )
					{
						iprice = iprice
					}else
					if( now.isBetween(date6,date12) )
					{
						iprice = iprice - (25 * iprice / 100)
					}else
					if( now.isBetween(date12,date18) )
					{
						iprice = iprice - (50 * iprice / 100)
					}else
					if( now.isBetween(date18,date24) )
					{
						iprice = iprice - (75 * iprice / 100)
					}else
					if( now.isAfter(date24) )
					{
						iprice = 0;
					}
			...
			$scope.reduced = function(initialPrice,date){
					date = moment(date);
					var date6 = date.add(6,'months');
					var date12 = date.add(12,'months');
					var date18 = date.add(18,'months');
