angular.module('pai-fabricant')
  .directive('fabricant', ["articleService", function (articleService){
    ...
    return {
		...
		replace: true,
		templateUrl: 'js/front/PAI/fabricant/fabricants.html',
		controller :["$scope", "fabricantService", function($scope,fabricantService){
			...
			$scope.myPagingFunction = function(){
				...
				console.log('PAGIN FUNCTION');
				if($scope.fetching == false){
					$scope.fetching = true;
					$scope.page = $scope.page + 1;
					console.log('PAGIN, '+$scope.page);
					fabricantService.fetch($scope.page).then(function(data){
						console.log('FETCH');
						// console.log(data);
						if(data.length)
