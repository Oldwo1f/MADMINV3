angular.module('pai-blog')
  .directive('addArticle', function (){
    ...
    return {
		...
		replace: true,
		templateUrl: 'js/front/PAI/blog/addArticle.html',
		controller :["Flash", "$window", "$state", "imageService", "tagService", "categoryService", "userService", "articleService", "$scope", "$rootScope", "$q", "Upload", "$sailsSocket", "$timeout", function(Flash,$window,$state,imageService,tagService,categoryService,userService,articleService, $scope, $rootScope, $q, Upload,$sailsSocket,$timeout){
			...
			$scope.finishedWizardAddContent = function(){
				...
				articleService.update($scope.formData.id,attrToUpdate).then(function(data){
					...
					collabspoints.contentModel = 'article'
					...
					userService.addCollabsPoints(userService.me.id,collabspoints).then(function(newcollabs){
						...
						articleService.addCollabsPoints($scope.formData.id,newcollabs.child.id).then(function(data){
	   ...
	   //      			$scope.nbPoints += 5;
        		...
        		}else if($scope.formData.contentType !='article'){
        			$scope.nbPoints += 10;
        			if($scope.formData.content ){
		                var val = $scope.formData.content.split(' ')
		        		if(val.length <100)
			...
			$scope.exitValidationContenus = function(){
				if($scope.formData.contentType =='link')
				{
					if($scope.formData.content && $scope.formData.title && $scope.formData.link && $scope.formData.tags.length)
					{
						return true;
					}else{
...
// saveImage
		...
		link:function(scope,element,attrs){
			...
			console.log('LINKKKKKKKKKKKK add article');
