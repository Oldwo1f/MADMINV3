...
.factory('projectService', function ($http,$q,$sailsSocket,$state,$auth) {
    ...
    service.fetch= function(page) {
        ...
        $sailsSocket.get('/marketplace/fetchItem/'+page).success(function (data,status) {
            console.log(data);
            service.items =data;
    ...
    service.addCollabsPoints=function(id, idCollabs){
        ...
        var deferred = $q.defer();
        $sailsSocket.post('/api/project/'+id+'/collabsPoints/'+idCollabs).success(function (data,status) {
            deferred.resolve(data);
        }).error(function (data,status) {
            // if(status == '401')
            //     $state.go('login')
