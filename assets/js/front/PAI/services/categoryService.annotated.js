angular.module('PAI').factory('categoryService',["$compile", "$sailsSocket", "$q", "$http", "$state", function ($compile,$sailsSocket,$q,$http,$state) {
    ...
    service.fetchAll= function(sort,page,nbPerPage) {
        ...
        $sailsSocket.get('/api/category?sort=name DESC&limit=10000').success(function (data,status) {
            deferred.resolve(data);
        }).error(function (data,status) {
