...
.directive('addIngrediant', function(){
    ...
    return {
      	...
      	controller:["$state", "$timeout", "$scope", "userService", "$rootScope", "ingrediantService", "$mdToast", "imageService", "documentService", "tagService", "categoryService", "$q", "Upload", "$sailsSocket", function($state,$timeout,$scope,userService,$rootScope,ingrediantService,$mdToast,imageService,documentService, tagService, categoryService, $q, Upload,$sailsSocket){
			...
			$scope.validateIngrediant=function(){
				$rootScope.startSpin();
				
				ingrediantService.validatePai($scope.formData.id).then(function(data){
					console.log(data);
